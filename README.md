...
- 拖动形状的过程中线不会实时跟着，松手后约 0.2 秒内跟上。拖线上的黄点是实时的。
- 每次自动跟随会在撤销历史里多一步。
- 连接线只认它记住的两个形状，形状删掉后线留在原地不再更新。
- 同一条边上有多条线时，默认都从边的中点出发，会重叠，拖端点错开即可。
- 线不会绕开无关的形状，只保证不穿过自己两端的形状。
- 旋转过的形状按外接矩形处理，不贴合真实轮廓。

## 工作原理

连接线本身是一个带自定义几何的普通 PowerPoint 形状，不是 PowerPoint 的连接线对象，圆角写在形状的几何定义里，由 PowerPoint 自己渲染，所以文件离开插件也能正常显示。八种走线（2 到 5 段）预先做成模板，插件按需复制到幻灯片上，之后只改位置、大小、翻转和调节值。

两端连着哪个形状、从哪条边、在边上的位置等信息存在形状的标签里。插件每 0.2 秒检查一次当前页，发现形状动了就更新相应的线。异形形状的轮廓通过把形状渲染成一张小图取样得到。
...
assets/                   图标
docs/                     README 用图
tools/build_template.py   生成八种走线的模板 template.pptx，编译时自动调用
build.cmd                 编译插件和安装程序
...
需要 [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)（勾选 MSBuild 工具）和 Python 3，Python 里装好 `python-pptx`。
